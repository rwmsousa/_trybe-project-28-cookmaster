...
const verifyQuantityArrayUsers = (itensSold) => {
...
const verifyStock = async (arrayUsersRecipe) => {
    const user = await usersModel.getUserIdModel(arrayUsersRecipe[0].userId);
    if (arrayUsersRecipe[0].quantity > user.quantity) {
        throw errorConstructor(404, 'Such amount is not permitted to sell');
    }
};
