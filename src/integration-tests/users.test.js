...
const userusersModel = require('../models/userusersModel');

const mongoDbUrl = `mongodb://${ process.env.HOST || 'mongodb' }:27017/FakeBank`;
const url = 'http://localhost:3000';

describe('USERS', () => {
  describe('Testa a criação de um novo usuario.', () => {
    ...
    before(async () => {
      ...
      const myobj = { name: 'Superman', email: 'maravilha@cripto.com', password: '123456' };
    ...
    const { name, quantity } = payloadUser;
    ...
    it('se retorna um objeto', async () => {
      ...
      const response = await usersModel.createUserModel(name, quantity);
  ...
  describe('Testa a busca de todos os usuarios.', () => {
    let connection;
    let db;
    ...
    it('se retorna um array de objetos', async () => {
      const response = await usersModel.getUserusersModel();
